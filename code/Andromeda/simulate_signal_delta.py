    ...
    for i, R in enumerate(R_list):
        ...
        try:
            distRX, distRY, distRC = np.loadtxt(
                dist_path
                + "distribution_radius_%.2f_%s%s%s%s.txt"
                % (R_kpc, PROFILE, circ_text, cut_text, IDstr),
                delimiter=", ",
                dtype="f8",
                usecols=(0, 1, 2),
                unpack=True,
            ...
            distMX, distMY = np.loadtxt(
                dist_path
                + "distribution_mass_%.2f_%s%s%s%s.txt"
                % (R_kpc, PROFILE, circ_text, cut_text, IDstr),
                delimiter=", ",
                unpack=True,
            )
...
for l, R in enumerate(tqdm(R_sample)):
    ...
    MC_r[l] = NE.inverse_transform_sampling_log(
        interp_r_corr, dist_r, n_samples=1, nbins=1000
    )
    ...
    MC_rho[l] = NE.inverse_transform_sampling_log(
        P_rho_given_r, [rho_min, rho_max], n_samples=1, nbins=1000
    )
...
int_file = dirs.data_dir + "Interaction_params_SJW_%s%s%s%s%s.txt.gz" % (
    PROFILE,
    circ_text,
    cut_text,
    pert_text,
